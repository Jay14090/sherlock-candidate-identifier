...
>
> Nine weak signals are combined by a weighted scoring engine. No single signal can dominate: name match is only 18%, because names lie. Missing data is scored neutral, never zero. Scores are temporally smoothed so one event can't cause a wild swing.
>
> Selection needs two things: top score ≥ 68% **and** a 12-point lead over the runner-up. Otherwise the system says 'uncertain' out loud.
>
...
> "Baseline first. Watch the status: it starts at 'Insufficient data' — the system refuses to decide on two events. As interviewers join early and the candidate joins right at the scheduled time, identity priors appear. When Jay introduces himself — 'my name is', 'I built' — the transcript signal kicks in and confidence crosses the threshold: selected at 87%, sixty-plus points clear of the interviewers. The explanation panel shows every piece of evidence with its exact point impact."

## 5. Live demo — MacBook Pro (4:30 – 6:30)

Scenario: **Candidate joins as MacBook Pro**. Press Play.

> "Now the fun one. The candidate joins as 'MacBook Pro' — the name signal is explicitly weak: 'appears to be a device name, but this participant is not ruled out.'
>
> The system stays uncertain until she starts answering: 'I am currently in my final year', 'I built a fraud detection dashboard'. Transcript role and speaking pattern push her up while the interviewer list pins Priya and Rohan down.
>
> Then she renames to 'Ananya S' mid-meeting — watch the card: name history 'MacBook Pro → Ananya S', the consistency signal calls out the rename converging with the candidate identity, and she's selected at 73%. No single rule did this; five weak signals agreed."
...
>
> Honest limitations: hand-tuned weights, seven authored scenarios, keyword matching can be paraphrased around. The evaluation — 7 out of 7 with correct abstention — validates the reasoning, not production accuracy."
