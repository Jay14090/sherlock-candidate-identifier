...
4. **Transcript role classification is intentionally lightweight in the default offline demo** and should be replaced by an embedding- or LLM-based classifier in production. The hybrid classifier's semantic stage (bag-of-words similarity against small example banks) handles common paraphrases but remains vulnerable to transcription noise, deliberate evasion, and vocabulary outside the example banks, and is English-only. A working Claude-backed classifier behind the same interface ships in the dashboard as an opt-in mode (`lib/classifiers/llmTranscriptClassifier.ts`), demonstrating the swap — but the default evaluation deliberately does not depend on it. Upgrade path: [scoring.md](scoring.md#transcript-classifier-roadmap); honest scope notes: [evaluation.md](evaluation.md#aiml-evaluation-notes).
